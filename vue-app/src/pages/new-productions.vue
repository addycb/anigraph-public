<template>
  <v-app>
    ...
    <v-main>
      <v-container fluid>
        <!-- Toolbar: Title + Type Filter + Release Toggle + Card Size -->
        <ViewToolbar
            ...
            <div class="d-flex align-center flex-wrap" style="gap: 12px;">
              <div class="page-title-section">
                <div class="d-flex align-center">
                  <v-icon color="primary" size="28" class="mr-2">mdi-new-box</v-icon>
                  <h1 class="text-h5 font-weight-bold mb-0">New Productions</h1>
                </div>
                <p class="text-caption text-medium-emphasis ml-9 mb-0">
                  Recently released anime and manga
                </p>
              </div>
              ...
              ></v-select>
              <v-btn
                :icon="hideUnreleased ? 'mdi-calendar-check' : 'mdi-calendar-clock'"
                :color="hideUnreleased ? 'primary' : 'default'"
                variant="outlined"
                density="compact"
                @click="toggleReleaseFilter"
              >
                <v-icon>{{ hideUnreleased ? 'mdi-calendar-check' : 'mdi-calendar-clock' }}</v-icon>
                <v-tooltip activator="parent" location="top">
                  {{ hideUnreleased ? 'Showing released only' : 'Showing all (including unreleased)' }}
                </v-tooltip>
              </v-btn>
            </div>
          </template>
        ...
        <v-row v-if="loading" class="mt-n2">
          <v-col cols="12" class="text-center py-12">
            <v-progress-circular
              ...
              size="64"
            ></v-progress-circular>
            <p class="text-h6 mt-4">Loading new productions...</p>
          </v-col>
        </v-row>
...
const hideUnreleased = ref(route.query.released !== 'false')
...
const toggleReleaseFilter = () => {
  hideUnreleased.value = !hideUnreleased.value
  applyFilters()
}
...
const fetchNewProductions = async (append = false) => {
  ...
  try {
    const params: any = {
      ...
      limit,
      offset: offset.value,
      releasedOnly: hideUnreleased.value
    }
    ...
    console.error('Error fetching new productions:', error)
  } finally {
    loading.value = false
...
const updateUrlQuery = () => {
  ...
  if (!hideUnreleased.value) query.released = 'false'
...
document.title = 'New Productions - Anigraph'
</script>
